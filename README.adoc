...
This add-on provides a custom datatype, a related field and a dataGrid column renderer.
...
You can define an entity attribute with the `MultilingualString` datatype using Studio. For example:
...
NOTE: The actual value stored in the database is a JSON string. For example: {"en":"Computer","el":"Ηλεκτρονικός υπολογιστής"}
